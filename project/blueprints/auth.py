...
def login():
    ...
    if request.method == 'POST':
        ...
        locked, attempts_remaining = is_account_locked(email)
        if locked:
            time_remaining = get_lockout_time_remaining(email)
            flash(f'⚠️ Cuenta bloqueada temporalmente por múltiples intentos fallidos. Inténtalo de nuevo en {time_remaining} minutos.', 'error')
        ...
        try:
            ...
            # 4. Resetear intentos fallidos al login exitoso
            ...
            if "Email not confirmed" in e.message:
                ...
                if locked:
                    flash(f'❌ Credenciales incorrectas. Cuenta bloqueada por 15 minutos.', 'error')
                else:
                    flash(f'❌ Email o contraseña incorrectos. Te quedan {attempts_remaining} intentos.', 'error')
            else:
